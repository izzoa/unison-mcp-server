...
- **clink: built-in job control — no MCP host tool timeout can kill a CLI run.** The host-proof answer to hosts whose tool deadlines are shorter than a real CLI run (Claude Desktop's local-agent mode cancels around a minute; an agentic Copilot review needs several): every `clink` call now waits a bounded time (`wait_seconds`, default 45s — under every known host deadline). Runs finishing in time return **byte-identical** results to the old blocking behavior, continuation offer included; longer runs return `status: "running"` with a `job_id`, and calling `clink` again with only that `job_id` continues the wait — the full original result (or the original error) is delivered the moment the CLI finishes. `job_id` + `cancel: true` aborts and kills the CLI's whole process tree. Background runs execute on isolated `CLinkTool` instances (the registry's cached singleton carries mutable per-request state), suppress their own progress notifications (heartbeats come from the foreground call actually waiting), respect a 3-job concurrency cap, support `idempotency_key` so a retried start never launches duplicate paid CLI work, retain results for 30 minutes with lazy GC, report "server restarted" honestly when in-memory state is gone, and are cancelled deterministically at server shutdown. `CLINK_EXECUTION_MODE=blocking` restores pure blocking. Supporting change: read-only verification snapshots run in a worker thread so background jobs never stall follow-up calls. (The first iteration of this shipped briefly in 14.6.0 as three separate tools — `clink_start`/`clink_poll`/`clink_cancel`; those are removed, and job control lives on `clink` itself.)
- **Setup seeds Claude's MCP tool timeout automatically.** When `run-server.sh` / `run-server.ps1` detect Claude (CLI or Desktop), they now add `MCP_TOOL_TIMEOUT=900000` (15 minutes) to the **top-level** `env` block of `~/.claude/settings.json` — the location the Claude client actually reads for its tool-call deadline, honored by both Claude Code CLI sessions and Claude Desktop's local sessions. The write is merge-preserving and backed up, creates the file when absent, skips (with a notice) when the variable is already set to any value, and leaves unparseable files untouched. This closes the out-of-box gap where long clink CLI runs died to the host's short default deadline while the server's own 3600s timeout never got a chance to matter.
- **clink: MCP progress heartbeats during CLI execution.** While a spawned CLI runs, clink now emits a progress notification every ~10 seconds via `ServerSession.report_progress` (message: which CLI, elapsed seconds). For hosts that never requested progress this is a guaranteed no-op; hosts that reset their tool timeout on progress keep multi-minute CLI runs alive; every other host at least surfaces liveness instead of a silent long call. The heartbeat is cancelled before the response is returned (the spec forbids progress after completion) and a notification failure can never break the CLI call. Docs also correct a long-standing error the investigation surfaced: `MCP_TOOL_TIMEOUT` belongs in the **top-level** `env` block of `~/.claude/settings.json` (or Desktop's local environment editor) — putting it under `mcpServers.<name>.env` hands it to the spawned *server* process, not the client enforcing the deadline, and does nothing.
...
- **clink read-only verification can no longer starve the CLI call itself.** Server logs from a live failure showed each read-only clink call spending 60–90 seconds walking a 50,000+-entry OneDrive repo *before the CLI even spawned* (and again after) — the pre/post verification snapshots used an unpruned, unbounded, syscall-heavy walk, so on Claude Desktop (whose local-agent-mode tool timeout is shorter than Claude Code's) every call was cancelled mid-walk and misread as "clink timing out." The walk now uses `os.scandir` (cached dirent data instead of 3–4 syscalls per entry), prunes bulk directories that consumed the entry cap while providing no meaningful coverage (`.git`, `node_modules`, virtualenvs, build outputs — documented tradeoff), and honors a wall-clock budget per snapshot (`CLINK_SNAPSHOT_BUDGET_SECONDS`, default 30s). Truncation is reported honestly: coverage reads `working_dir_subtree (partial)` and a new `read_only_verification_stats` metadata block carries entry counts and elapsed times.
