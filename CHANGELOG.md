...
- **Claude Desktop is now detected on machines that haven't configured MCP yet.** `run-server.ps1` detected the app by the presence of `claude_desktop_config.json` — a file Claude Desktop only creates once MCP has been configured, i.e. the exact artifact the integration exists to write. Every fresh install reported "Claude Desktop not detected" (observed on a machine with the app installed). Detection now keys on the `%APPDATA%\Claude` app-data directory (created on first launch); the config writer already creates the file and directory when missing. The dead `NeedsConfigDir` metadata flag was removed.
