...
- **clink job API: `clink_start` / `clink_poll` / `clink_cancel`.** The host-proof answer to MCP hosts whose tool timeouts are shorter than a real CLI run (Claude Desktop's local-agent mode cancels around a minute; an agentic Copilot review needs several): no single MCP request outlives the host. `clink_start` takes the same arguments as `clink` and returns within ~1 second — runs finishing inside the grace window (including validation errors) return their result directly, longer runs return a `job_id`; `clink_poll` long-polls (bounded at 25s, under any host deadline) and embeds the full clink result on completion; `clink_cancel` kills the CLI's whole process tree. Jobs run on isolated `CLinkTool` instances (the registry's cached singleton carries mutable per-request state), suppress progress notifications (their originating request already completed), respect a 3-job concurrency cap, support an `idempotency_key` so a retried start never launches duplicate paid CLI work, retain results for 30 minutes with lazy GC, report "server restarted" honestly when in-memory state is gone, and are cancelled deterministically at server shutdown. Supporting change: the read-only verification snapshots now run in a worker thread, so background jobs can never stall `clink_poll` responses.
