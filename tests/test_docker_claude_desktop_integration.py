...
class TestDockerClaudeDesktopIntegration:
    ...
    def test_mcp_config_docker_run_format(self):
        ...
        assert "unison-mcp" in config["mcpServers"]
        assert config["mcpServers"]["unison-mcp"]["command"] == "docker"

        args = config["mcpServers"]["unison-mcp"]["args"]
        assert "run" in args
        assert "--rm" in args
        assert "-i" in args
        ...
        config = {
            "mcpServers": {
                "unison-mcp": {
                    ...
                    "args": ["-f", "/path/to/docker-compose.yml", "run", "--rm", "unison-mcp"],
                }
            }
        }

        # Validate configuration structure
        assert config["mcpServers"]["unison-mcp"]["command"] == "docker-compose"

        args = config["mcpServers"]["unison-mcp"]["args"]
        assert "-f" in args
        assert "run" in args
        assert "--rm" in args
        assert "unison-mcp" in args
    ...
    def test_windows_path_format(self):
            ...
            "mcpServers": {
                "unison-mcp": {
                    ...
                    "args": [
                        ...
                        "-i",
                        "--env-file",
                        "C:/Users/User/unison-mcp-server/.env",
                        "-v",
                        "C:/Users/User/unison-mcp-server/logs:/app/logs",
                        "unison-mcp-server:latest",
                    ],
                }
            }
        ...
        args = windows_config["mcpServers"]["unison-mcp"]["args"]
    ...
    def test_docker_image_reference(self):
        ...
        configs = [
            {"image": "unison-mcp-server:latest"},
            {"image": "unison-mcp-server:v1.0.0"},
            {"image": "registry/unison-mcp-server:latest"},
        ]
    ...
    def temp_mcp_config(self):
        ...
        config = {
            "mcpServers": {
                "unison-mcp": {
                    "command": "docker",
                    "args": ["run", "--rm", "-i", "--env-file", "/tmp/.env", "unison-mcp-server:latest"],
                }
            }
        }
...
class TestDockerMCPErrorHandling:
    ...
    def test_missing_docker_image_handling(self):
        ...
        nonexistent_config = {
            "mcpServers": {"unison-mcp": {"command": "docker", "args": ["run", "--rm", "-i", "nonexistent:latest"]}}
        }

        # Configuration should be valid even if image doesn't exist
        assert "unison-mcp" in nonexistent_config["mcpServers"]
    ...
    def test_invalid_env_file_path(self):
        ...
        config_with_invalid_env = {
            "mcpServers": {
                "unison-mcp": {
                    "command": "docker",
                    "args": ["run", "--rm", "-i", "--env-file", "/nonexistent/.env", "unison-mcp-server:latest"],
                }
            }
        }

        # Configuration structure should still be valid
        args = config_with_invalid_env["mcpServers"]["unison-mcp"]["args"]
        assert "--env-file" in args
    ...
    def test_resource_limit_configurations(self):
        ...
        config_with_limits = {
            "mcpServers": {
                "unison-mcp": {
                    "command": "docker",
                    "args": ["run", "--rm", "-i", "--memory=512m", "--cpus=1.0", "unison-mcp-server:latest"],
                }
            }
        }

        args = config_with_limits["mcpServers"]["unison-mcp"]["args"]
