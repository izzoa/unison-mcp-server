...
Unison MCP Server centers on `server.py`, which exposes MCP entrypoints and coordinates multi-model workflows. 
...
- `source .unison_venv/bin/activate` – activate the managed Python environment.
...
```bash
.unison_venv/bin/activate && ./code_quality_checks.sh
```

For example, this is how we run an individual / all tests:

```bash
.unison_venv/bin/activate && pytest tests/test_auto_mode_model_listing.py -q
.unison_venv/bin/activate && pytest -q
```
