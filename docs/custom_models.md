...
This guide covers setting up multiple AI model providers including OpenRouter, custom API endpoints, and local model servers. The Unison MCP server supports a unified configuration for all these providers through a single model registry.
...
Unison ships multiple registries:
...
The Unison MCP server runs natively, so you can use standard localhost URLs to connect to local models:
...
"Use pro via unison to analyze this"    # → google/gemini-2.5-pro
"Use gpt4o via unison to analyze this"  # → openai/gpt-4o
"Use mistral via unison to optimize"    # → mistral/mistral-large
...
```
# OpenRouter models:
"Use anthropic/claude-opus-4 via unison for deep analysis"
"Use openai/gpt-4o via unison to debug this"
"Use deepseek/deepseek-coder via unison to generate code"

# Local/custom models:
"Use llama3.2 via unison to review this"
"Use meta-llama/Llama-2-7b-chat-hf via unison to analyze"
```
