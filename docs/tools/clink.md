...
### Long runs under impatient hosts: the job API

When the host's tool timeout is shorter than a real CLI run (Claude Desktop's local-agent mode cancels around a minute; a serious Copilot review needs several), no amount of server-side tuning can save a single long MCP request. Use the job API instead — no single request ever outlives the host's patience:

- **`clink_start`** — same arguments as `clink` (plus optional `idempotency_key`). Runs finishing within ~1s (including validation errors) return their result directly; anything longer returns a `job_id` immediately.
- **`clink_poll`** — pass the `job_id`; long-polls up to `wait_ms` (default 20s, max 25s — always under host deadlines) and returns the status. On `completed`, the full clink result is embedded under `result`; failures appear under `error`. Keep polling until a terminal status.
- **`clink_cancel`** — cancels the job and kills the CLI's entire process tree.

Job state is in-memory (results retained 30 minutes; up to 3 concurrent jobs; retrying `clink_start` with the same `idempotency_key` returns the running job instead of launching duplicate paid work). If the MCP server restarts between calls, `clink_poll` says so explicitly. Each job runs on an isolated tool instance and suppresses progress notifications (its originating request has already completed).
