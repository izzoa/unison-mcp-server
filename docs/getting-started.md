# Getting Started with Unison MCP Server

This guide walks you through setting up the Unison MCP Server from scratch, including installation, configuration, and first usage.
...
- Generate an API key for native Claude access (Fable 5, Opus 5/4.8, Sonnet 5/4.6, Haiku 4.5) with extended thinking
...
```json
{
  "mcpServers": {
    "unison": {
      ...
      "args": [
        "-c", 
        "for p in $(which uvx 2>/dev/null) $HOME/.local/bin/uvx /opt/homebrew/bin/uvx /usr/local/bin/uvx uvx; do [ -x \"$p\" ] && exec \"$p\" --from git+https://github.com/izzoa/unison-mcp-server.git unison-mcp-server; done; echo 'uvx not found' >&2; exit 1"
      ],
      "env": {
...
Enable Codex's built-in web-search tool so Unison's `apilookup` instructions can execute successfully:
...
```json
{
  ...
  "mcp": {
    "unison": {
      "type": "local",
      "command": [
        "/path/to/unison-mcp-server/.unison_venv/bin/python",
        "/path/to/unison-mcp-server/server.py"
      ],
      "cwd": "/path/to/unison-mcp-server",
      "enabled": true,
      "environment": {
...
Unison works in GUI IDEs that speak MCP. The configuration mirrors the CLI examples above—point the client at the `uvx` launcher and set any required environment variables.
...
2. Click **Add MCP Server** and supply the following values:
   - Command: `sh`
   - Args: `-c` and `for p in $(which uvx 2>/dev/null) $HOME/.local/bin/uvx /opt/homebrew/bin/uvx /usr/local/bin/uvx uvx; do [ -x "$p" ] && exec "$p" --from git+https://github.com/izzoa/unison-mcp-server.git unison-mcp-server; done; echo 'uvx not found' >&2; exit 1`
   - Environment (example):
     - `PATH=/usr/local/bin:/usr/bin:/bin:/opt/homebrew/bin:~/.local/bin`
     - `GEMINI_API_KEY=your_api_key_here`
...
👉 Pro tip: If you prefer a one-line command, replace the long loop with `uvx --from git+https://github.com/izzoa/unison-mcp-server.git unison-mcp-server`—just make sure `uvx` is on your PATH for every client.
...
ANTHROPIC_API_KEY=your-anthropic-api-key-here  # For native Claude (Fable, Opus, Sonnet, Haiku)
...
Some MCP clients default to short timeouts and can disconnect from Unison during long tool runs. Configure each client with a generous ceiling (we recommend at least five minutes); the Unison setup script now writes a 20-minute tool timeout for Codex so upstream providers contacted by the server have time to respond.
...
3. Try: `"Use unison to list available models"`

### For Claude Code CLI:
1. Exit any existing Claude session
2. Run `claude` from your project directory  
3. Try: `"Use unison to chat about Python best practices"`

### For Gemini CLI:
**Note**: While Unison MCP connects to Gemini CLI, tool invocation isn't working correctly yet. See [Gemini CLI Setup](gemini-setup.md) for updates.

### For Qwen Code CLI:
1. Restart the Qwen Code CLI if it's running (`qwen exit`).
2. Run `qwen mcp list --scope user` and confirm `unison` shows `CONNECTED`.
3. Try: `"/mcp"` to inspect available tools or `"Use unison to analyze this repo"`.

### For OpenCode CLI:
1. Restart OpenCode (or run `OpenCode: Reload Config`).
2. Open **Settings › Tools › MCP** and confirm `unison` is enabled.
3. Start a new chat and try: `"Use unison to list available models"`.
...
### Test Commands:
```
"Use unison to list available models"
"Chat with unison about the best approach for API design"
"Use unison thinkdeep with gemini pro about scaling strategies"  
...
## Step 5: Start Using Unison

### Basic Usage Patterns:

**Let Claude pick the model:**
```
"Use unison to analyze this code for security issues"
"Debug this race condition with unison"
"Plan the database migration with unison"
```

**Specify the model:**
```  
"Use unison with gemini pro to review this complex algorithm"
"Debug with o3 using unison for logical analysis"
"Get flash to quickly format this code via unison"
```

**Multi-model workflows:**
```
"Use unison to get consensus from pro and o3 on this architecture"
...
### "unison not found" or "command not found"
...
- Check virtual environment: `which python` should show `.unison_venv/bin/python`
...
**"Model not available":**
- Run `"Use unison to list available models"` to see what's configured
